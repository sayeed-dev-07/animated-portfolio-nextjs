...
export default function TextBorderAnimation({ text = "Programming", className }: TextProps) {
  ...
  return (
    <div onMouseEnter={handleHover} onMouseLeave={handleHoverExit} className="overflow-hidden">
      <span className={cn("md:text-4xl text-2xl font-semibold text-foreground wrap-break-word", className)}>{text}</span>
      <div className="relative mt-1 h-1 w-full">
        <div
          className={cn(
            "absolute left-0 top-0 h-full w-full bg-foreground transition-transform duration-300",
            isHoveredIn
        ...
        <div
          className={cn(
            "absolute left-0 top-0 h-full w-full translate-x-0 transform bg-foreground opacity-0 transition-transform duration-300",
